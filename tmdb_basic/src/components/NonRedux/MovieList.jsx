...
function MovieList() {
   ...
   useEffect(() => {
      const fetchMovies = async () => {
         try {
            ...
            setError(error.message) // error state에 에러메시지 저장
         } finally {
   ...
   //   error state에 에러메세지가 있으면 error 메시지 컴포넌트를 렌더링
   if (error) return <p>Error: {error}</p>

   // loading state가 false 면서 error state에 에러메세지가 없으면 영화 목록 컴포넌트를 렌더링
   return (
      <div>
         <h1>인기 영화 목록</h1>
         <ul>
